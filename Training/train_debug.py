...
def main():
    ...
    EPSILON_DECAY = 1 # Decay rate
    DISCOUNT = 0.90
    MIN_EPSILON = 0.01 # Minimum exploration rate
    UPDATE_TARGET_INTERVAL = 500

    df = pd.read_csv("../Data/dataset/VETUSDT.csv")
    sequences = create_lookback_batches(df.iloc[:100], LOOPBACK_WINDOW)
    ...
    for episode in progressbar:
        ...
        while not done:
            ...
            if (r <= EPSILON):
                ...
                time_choose_action  = end_time_prediction - start_time_prediction
        ...
        if len(agent.memory) > BATCH_SIZE:
            ...
            start_time_prediction_1 = time.time()
            current_state_q_values = agent.predict_network_q(mini_batch_states)
            end_time_prediction_1 = time.time()
            time_y_values  = end_time_prediction_1 - start_time_prediction_1
            y = current_state_q_values

            start_time_prediction_target_1 = time.time()
            next_state_q_values = agent.predict_network_target(mini_batch_next_state)
            end_time_prediction_target_1 = time.time()
            time_next_y_values  = end_time_prediction_target_1 - start_time_prediction_target_1
            ...
            time_fit_q  = end_time_fit_q - start_time_fit_q
    ...
    print(f"Checkpoint 1: {time_choose_action}, Checkpoint 2: {time_y_values}, Checkpoint 3: {time_next_y_values}, Checkpoint 4: {time_fit_q}")
